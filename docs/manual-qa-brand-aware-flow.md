...
5. На review-экране для `[DEMO] Freeform — на ревью`:
   - **Структура итогового брифа** (секция вверху): дерево разделов/полей редактируется
     чекбоксами; «Сохранить структуру» (`POST .../select-template`). У seed-брифа без шаблона —
     CTA «Использовать стандартную структуру».
...
ведёт на `/review`; видны структура → summary → поля → уточнения; бейджи читаемы; generate-final
гейтится при критичных полях.
