...
E2E-тесты живут в `frontend/e2e` (только Chromium, локально, без CI). Playwright сам
поднимает frontend (vite на :5173); backend и demo-данные — внешний prerequisite:

```bash
# 1) backend на localhost:8000, dev-БД мигрирована (alembic upgrade head → 0005)
# 2) детерминированные demo-данные:
cd backend && python scripts/seed_demo.py --reset
# 3) E2E (первый раз — npx playwright install chromium):
cd ../frontend && npm run test:e2e          # headless
#                  npm run test:e2e:headed   # с окном
#                  npm run test:e2e:ui       # UI mode
```

Реальный `LLM_API_KEY` не нужен — тесты опираются на pre-generated `[DEMO]`-брифы из seed.

**Покрыто:** рендер `/` / `/briefs` / `/brands`; wizard-routing (редактор, не review);
freeform review-степпер из 5 шагов; template editor; шаги summary / structured / clarifications /
final; скачивание md / json / docx; outdated-состояние без LLM; DOCX-паритет в wizard.
**Не покрыто (сознательно):** реальная LLM-генерация, PDF, brand identity, visual regression,
CI-матрица. Подробности — в [frontend/e2e/README.md](frontend/e2e/README.md).
